...
<img :src="$withBase('/images/start_shizuku.png')" style="max-width:240px;width:101%">
...
3. Enter ``, if success, you can see a long list of content instead of the prompt not finding adb.
...
2. If you use PowerShell or Linux/Mac, all `` should be replaced with `./`
:::


Setting ``

To use `` you first need to turn on USB debugging on your device, usually by following these steps:
...
4. Connect the device to the computer and type `devices` in the terminal.
5. At this time, the dialog "Allow debugging" will appear on the device, check "Always allow" and confirm.
6. Enter `devices` again in the terminal. If there is no problem, you will see something like the following.
...
Many manufacturers have made modifications to the Android system that prevent Shizuku from working properly.Start via wireless debugging: keeps showing "Searching for pairing service"
